-- Phase 1 Complete Implementation: Data Integrity & Validation Foundation
...
-- Step 2: Create comprehensive data validation function
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Count shopify sync status records
      SELECT COALESCE(products_synced, 0) INTO shopify_status_count
...
-- Step 3: Create comprehensive cleanup function
...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      -- 1. Sync marketplace_sync_status with shopify_sync_status
      IF NOT dry_run THEN
        INSERT INTO public.marketplace_sync_status (
          user_id, marketplace, sync_status, products_synced, last_sync_at, 
          total_products_found, active_products_synced, inactive_products_skipped
        )
        SELECT 
          sss.user_id, 'shopify', sss.sync_status, sss.products_synced, sss.last_sync_at,
          sss.total_products_found, sss.active_products_synced, sss.inactive_products_skipped
        ...
        DO UPDATE SET
          ...
          inactive_products_skipped = EXCLUDED.inactive_products_skipped,
          updated_at = now();
      END IF;
