-- First, let's delete products and orders that reference the orphaned stores
DELETE FROM products WHERE store_id IN (
  SELECT id FROM store_configurations 
  WHERE store_name IN ('Shopi', 'Prohair S') AND platform = 'shopify'
);

-- Delete orders that reference these stores by store_name
DELETE FROM orders WHERE store_name IN ('Shopi', 'Prohair S');

-- Now we can safely delete the orphaned store configurations
DELETE FROM store_configurations WHERE store_name IN ('Shopi', 'Prohair S') AND platform = 'shopify';

-- Update the cascade delete function to handle foreign key relationships properly
