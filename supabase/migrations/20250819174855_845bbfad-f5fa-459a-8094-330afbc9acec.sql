...
-- First, check what columns exist in orders table to find the store reference
-- Let's delete the orphaned store configurations directly since they're no longer needed
...
RETURNS TRIGGER AS $$
BEGIN
  -- Delete all orders from this store (using store_vendor if it exists)
  DELETE FROM public.orders WHERE store_vendor = OLD.store_name AND user_id = OLD.user_id;
  
  -- Delete all products from this store (using vendor column)
...
-- Note: We cannot create triggers on auth.users as it's managed by Supabase
-- Instead, we'll handle user deletion through the application layer