...
export const ProductTypeGenerator = ({ 
  ...
  const generateProductTypes = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ai-generate-product-types', {
        body: { products: selectedProductsList }
      });
        ...
        <CardContent className="space-y-4">
          <div className="max-h-96 overflow-y-auto space-y-3">
            {suggestedChanges.map((change) => (
              <div key={change.id} className="p-4 border rounded-lg space-y-2">
  ...
  return (
    <Card className="border-0 shadow-card">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Wand2 className="h-5 w-5" />
          AI Product Type Generator
        </CardTitle>
        <CardDescription>
          Generate specific product types for selected products using AI
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
